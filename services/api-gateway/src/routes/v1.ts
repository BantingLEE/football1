...
const LEAGUE_SERVICE_URL = process.env.LEAGUE_SERVICE_URL || 'http://localhost:3006'
const YOUTH_SERVICE_URL = process.env.YOUTH_SERVICE_URL || 'http://localhost:3007'
const MESSAGE_SERVICE_URL = process.env.MESSAGE_SERVICE_URL || 'http://localhost:3008'
...
router.use('/economy', authMiddleware, createProxyMiddleware({
  ...proxyOptions,
  target: ECONOMY_SERVICE_URL,
  pathRewrite: { '^/api/v1/economy': '/economy' }
}))

router.use('/leagues', authMiddleware, createProxyMiddleware({
  ...proxyOptions,
  target: LEAGUE_SERVICE_URL,
  pathRewrite: { '^/api/v1/leagues': '/leagues' }
}))

router.use('/youth', authMiddleware, createProxyMiddleware({
  ...proxyOptions,
